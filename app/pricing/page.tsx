...
function CheckIcon({ className }: { className: string }) {
  return (
    <svg className={`w-5 h-5 ${className} mt-0.5`} fill="currentColor" viewBox="0 0 20 20">
...
function PricingPageContent() {
  ...
  const isProMonthly = isPro && subscription?.billing_interval === 'month';
  const isProAnnual = isPro && subscription?.billing_interval === 'year';
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid md:grid-cols-3 gap-8 max-w-5xl mx-auto items-start">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-8 border border-gray-200 dark:border-gray-700">
            ...
            <button
              onClick={() => handleUpgrade(monthlyPriceId, 'month')}
              disabled={loadingPlan !== null || isPro}
              className="w-full py-3 px-4 bg-indigo-600 text-white rounded-lg font-bold hover:bg-indigo-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {isProMonthly ? 'Current Plan' : isPro ? 'Pro Monthly' : loadingPlan === 'month' ? 'Loading...' : 'Get Pro Monthly'}
            </button>
          </div>
          ...
          <div className="bg-gradient-to-br from-indigo-600 to-purple-600 rounded-xl shadow-2xl p-8 border-2 border-indigo-400 relative">
            ...
            <button
              onClick={() => handleUpgrade(annualPriceId, 'year')}
              disabled={loadingPlan !== null || isPro}
            ...
            >
              {isProAnnual ? 'Current Plan' : isPro ? 'Pro Annual' : loadingPlan === 'year' ? 'Loading...' : 'Get Pro Annual'}
            </button>
          </div>
        </div>
